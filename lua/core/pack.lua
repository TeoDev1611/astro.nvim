...
local data_dir = utils.path_join(vim.fn.stdpath 'data', 'site')
local modules_dir = utils.path_join(vim_path, 'lua', 'modules')
local packer_compiled = utils.path_join(data_dir, 'lua', 'packer_compiled.lua')
local packer = nil
...
function Packer:load_plugins()
  ...
  local get_plugins_list = function()
    local list = {}
    local tmp = vim.split(
      vim.fn.globpath(utils.path_join(vim.fn.stdpath 'config', 'lua', 'modules'), '*/plugins.lua'),
      '\n'
    )
    for _, f in ipairs(tmp) do
      list[#list + 1] = string.match(f, utils.path_join('lua', '(.+).lua$'))
    end
    return list
  end
